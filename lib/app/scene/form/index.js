...
class FormAbortedCase extends case_1.Case {
    constructor(listener) {
        super(FormAborted, msg => future_1.doFuture(function* () {
            yield future_1.wrap(listener.afterFormAborted(msg));
...
class FormSavedCase extends case_1.Case {
    constructor(listener) {
        super(FormSaved, msg => future_1.doFuture(function* () {
            yield future_1.wrap(listener.afterFormSaved(msg));
            return future_1.voidPure;
        }));
        this.listener = listener;
    }
...
class SaveCase extends case_1.Case {
    constructor(form) {
        super(Save, (_) => future_1.wrap(form.save()));
        this.form = form;
    }
}
...
class SaveFailedCase extends case_1.Case {
    constructor(listener) {
        super(SaveFailed, fail => future_1.doFuture(function* () {
            yield future_1.wrap(listener.onSaveFailed(fail));
            yield future_1.wrap(listener.onSaveFinished());
            return future_1.voidPure;
        }));
        this.listener = listener;
    }
...
class SaveOkCase extends case_1.Case {
    constructor(form) {
        super(SaveOk, (ok) => future_1.doFuture(function* () {
            yield future_1.wrap(form.onSaveOk(ok));
            yield future_1.wrap(form.onSaveFinished());
...
class BaseFormScene extends __1.BaseAppScene {
    ...
    getModifiedValues() {
        return record_1.filter(this.value, (_, k) => array_1.contains(this.fieldsModifed, k));
    }
    onSaveFailed(_) { }
    onSaveOk(_) { }
    onSaveFinished() { }
